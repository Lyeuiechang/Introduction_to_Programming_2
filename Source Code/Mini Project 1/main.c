...
// Find the empty register.
void findEmptyReg(AST *now);
	...
	switch (root->kind)
	{
		case ASSIGN:
			...
			if(root->lhs->kind == IDENTIFIER)
			{
				if(root->lhs->val == 'x')
				{
					strcpy(VariableMemary,"[0]");
					Loadx = 1;
				}
				else if(root->lhs->val == 'y')
				{
					strcpy(VariableMemary,"[4]");
					Loady = 1;
				}
				else if(root->lhs->val == 'z')
				{
					strcpy(VariableMemary,"[8]");
					Loadz = 1;
				}
			}
			if(root->rhs->kind == CONSTANT)
			{
				findEmptyReg(root->rhs);
				printf("add r%d 0 %d\n",root->rhs->reg,root->rhs->val);
				printf("store %s r%d\n",VariableMemary,root->rhs->reg);
			}
			else if(root->rhs->kind == IDENTIFIER){
				printf("store %s r%d\n",VariableMemary,root->rhs->reg);
			}
			while(PostIncDec)
			{
				if('u' <= root->lhs->val && root->lhs->val <= 'w')
				{
					PostIncDec--;
					printf("add r%d r%d 1\n",root->lhs->reg,root->lhs->reg);
				}
				else if('r' <= root->lhs->val && root->lhs->val <= 't')
				{
					PostIncDec--;
					printf("sub r%d r%d 1\n",root->lhs->reg,root->lhs->reg);
				}
				if('u' <= root->rhs->val && root->rhs->val <= 'w')
				{
					PostIncDec--;
					printf("add r%d r%d 1\n",root->rhs->reg,root->rhs->reg);
				}
				else if('r' <= root->rhs->val && root->rhs->val <= 't')
				{
					PostIncDec--;
					printf("sub r%d r%d 1\n",root->lhs->reg,root->lhs->reg);
				}
		...
		case ADD:
			...
			else if(root->lhs->kind == IDENTIFIER && root->rhs->kind == IDENTIFIER)
			{
				findEmptyReg(root);
				printf("add r%d r%d r%d\n",root->reg,root->lhs->reg,root->rhs->reg);
				root->kind = IDENTIFIER;
			}
			else if(root->lhs->kind == CONSTANT && root->rhs->kind == IDENTIFIER)
			{
				findEmptyReg(root);
				printf("add r%d %d r%d\n",root->reg,root->lhs->val,root->rhs->reg);
				root->kind = IDENTIFIER;
			}
			else if(root->lhs->kind == IDENTIFIER && root->rhs->kind == CONSTANT)
			{
				findEmptyReg(root);
				printf("add r%d r%d %d\n",root->reg,root->lhs->reg,root->rhs->val);
				root->kind = IDENTIFIER;
			}
		...
		case MUL:
			...
			{
				findEmptyReg(root);
				printf("mul r%d r%d %d\n",root->reg,root->rhs->reg,root->lhs->val);
				root->kind = IDENTIFIER;
			}
			else if(root->lhs->kind == IDENTIFIER && root->rhs->kind == CONSTANT)
			{
		...
		case DIV:
			...
			while(PostIncDec)
			{
				if('u' <= root->lhs->kind && root->lhs->kind <= 'w')
				{
					PostIncDec--;
					printf("add r%d r%d 1\n",root->lhs->reg,root->lhs->reg);
				}
				else if('r' <= root->lhs->kind && root->lhs->kind <= 't')
				{
					PostIncDec--;
					printf("sub r%d r%d 1\n",root->lhs->reg,root->lhs->reg);
				}
				if('u' <= root->rhs->kind && root->rhs->kind <= 'w')
				{
					PostIncDec--;
					printf("add r%d r%d 1\n",root->rhs->reg,root->rhs->reg);
				}
				else if('r' <= root->rhs->kind && root->rhs->kind <= 't')
				{
					PostIncDec--;
					printf("sub r%d r%d 1\n",root->rhs->reg,root->rhs->reg);
				}
		...
		case POSTINC:
			...
			codegen(root->mid);
			root->kind = IDENTIFIER;
			root->val = root->mid->val-3;
			root->mid->kind = POSTINC;
			root->reg = root->mid->reg;
			break;
		case POSTDEC:
			PostIncDec++;
			codegen(root->mid);
			root->kind = IDENTIFIER;
			root->val = root->mid->val-6;
			root->mid->kind = POSTDEC;
			root->reg = root->mid->reg;
			break;
		case IDENTIFIER:
			if(root->val == 'x')
			{
				root->reg = 0;
				regSet[0] = 1;
				if((xLoaded == 0 && VariableSkiped == 0)||Loadx == 1)
				{
					printf("load %s %s\n","r0","[0]");
					xLoaded++;
					Loadx = 0;
			...
			else if(root->val == 'y')
			{
				root->reg = 1;
				regSet[1] = 1;
				if((yLoaded == 0 && VariableSkiped == 0)||Loady == 1)
				{
					printf("load %s %s\n","r1","[4]");
					yLoaded++;
					Loady = 0;
			...
			else if(root->val == 'z')
			{
				root->reg = 2;
				regSet[2] = 1;
				if((zLoaded ==0 && VariableSkiped ==0)||Loadz ==1)
				{
					printf("load %s %s\n","r2","[8]");
					zLoaded++;
					Loadz = 0;
		...
		case PLUS:
			...
			while(PostIncDec)
			{
				if('u' <= root->mid->val && root->mid->val <= 'w')
				{
					PostIncDec--;
					printf("add r%d r%d 1\n",root->mid->reg,root->mid->reg);
				}
				else if('r' <= root->mid->val && root->mid->val <= 't')
				{
					PostIncDec--;
					printf("sub r%d r%d 1\n",root->mid->reg,root->mid->reg);
				}
